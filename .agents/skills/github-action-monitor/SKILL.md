...
3. Use the `run_command` tool to execute `gh run list --limit 1 --json databaseId -q ".[0].databaseId"` to get the latest run ID.
4. Then use `run_command` to execute `gh run watch <run-id> --exit-status`. Because CI/CD pipelines can take several minutes, the command will automatically run as a background task. **DO NOT poll the task status manually in a loop.** Simply stop calling tools and end your turn. The system will automatically wake you up and notify you when the command completes.
